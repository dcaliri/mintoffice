...
class CommuteTest < ActiveSupport::TestCase
  fixtures :accounts, :people, :employees
  ...
  test "Commute can duplicated from yesterday to today" do
    ...
    before = account.employee.commutes.build(go: yesterday)
    after = account.employee.commutes.build(go: today)
  ...
  test "Commute can't duplicated between the attend" do
    ...
    before = account.employee.commutes.build(go: before_attend)
    after = account.employee.commutes.build(go: after_attend)
