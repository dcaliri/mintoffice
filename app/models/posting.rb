...
class Posting < ActiveRecord::Base
  ...
  def check_total_amount
    list = items.to_a
    credit_list = list.select{|item| item.item_type == '차변'}.sum(&:amount)    # models.posting.debit
    debit_list = list.select{|item| item.item_type == '대변'}.sum(&:amount)     # models.posting.credit
    unless credit_list == debit_list
      errors.add(:total_amount, "의 합계가 맞지 않습니다")        # models.posting.incorrect
    end
  end
