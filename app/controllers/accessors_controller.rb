...
class AccessorsController < ApplicationController
  def create
    ...
    redirect_to :back, notice: "성공적으로 권한을 설정하였습니다." # controllers.accessors.set_permission
  end

  def destroy
		person = AccessPerson.find(params[:accessor_id])
		person.destroy
		redirect_to :back, notice: "성공적으로 권한을 제거하였습니다." # controllers.accessors.delete_permission
  end
end